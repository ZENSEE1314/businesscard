...
import { storeImage, storeVideo } from "@/lib/storage/storage";
...
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    enforceRateLimit(`upload:${user.id}:${getClientIp(req)}`, 60, 60 * 60 * 1000);
    ...
    if (!FOLDERS.has(folder)) throw Errors.badRequest("Invalid upload target.");

    const isVideo = file.type.startsWith("video/");

    if (isVideo) {
      const stored = await storeVideo(file, folder).catch((e) => {
